...
export default function OrdersPage({ onError, onSuccess }) {
    ...
    return <div>
        <div className="card">
            ...
            <div className="minimal-list">{products.map(p => <label key={p.id} className="minimal-item"><input type="checkbox" checked={form.productIds.includes(p.id)} onChange={() => toggleProduct(p.id)} /><span>{p.name}</span></label>)}</div>
        ...
        <table className="table"><thead><tr><th>Status</th><th>Total</th><th>Customer</th><th>Products</th><th>Actions</th></tr></thead><tbody>
        {paged.map(o => <tr key={o.id}><td><span className={`badge status status-${String(o.status || '').toLowerCase()}`}>{o.status}</span></td><td>{Number(o.totalAmount || 0).toFixed(2)}</td><td>{o.customer?.name}</td><td>{(o.products || []).map(p => <span className="badge category" key={p.id}>{p.name}</span>)}</td><td><button className="btn btn-delete" onClick={async () => { try { await orderService.remove(o.id); onSuccess('Order deleted'); loadOrders(); } catch (e) { onError(e.message); } }}>Delete</button></td></tr>)}
        </tbody></table>
    </div>;
}