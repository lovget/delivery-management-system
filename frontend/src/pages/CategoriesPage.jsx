...
export default function CategoriesPage({ onError, onSuccess }) {
    ...
    return <div>
        ...
        {loading ? <div className="card empty-state">Loading categories...</div> : <div className="table-wrap"><table className="table table-categories"><colgroup><col className="name-col" /><col className="actions-col" /></colgroup><thead><tr><th>Name</th><th className="actions-col">Actions</th></tr></thead><tbody>
        {paged.length === 0 ? <tr><td colSpan={2} className="empty-state">Нет данных</td></tr> : paged.map(c => <tr key={c.id}><td>{editingId === c.id ? <input value={editName} onChange={(e) => setEditName(e.target.value)} /> : c.name}</td><td className="actions-cell"><div className="actions">{editingId === c.id ? <button className="btn btn-edit" onClick={() => saveEdit(c.id)}>Save</button> : <button className="btn btn-edit" onClick={() => startEdit(c)}>Edit</button>} <button className="btn btn-delete" onClick={() => remove(c.id)}>Delete</button></div></td></tr>)}
        </tbody></table></div>}
    </div>;
}