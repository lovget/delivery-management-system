...
export default function ProductsPage({ onError, onSuccess }) {
    ...
    return <div>
        ...
        {paged.map(p => <tr key={p.id}><td>{editingId===p.id ? <input value={editForm.name} onChange={(e)=>setEditForm({...editForm,name:e.target.value})}/> : p.name}</td><td>{editingId===p.id ? <input type="number" value={editForm.price} onChange={(e)=>setEditForm({...editForm,price:e.target.value})}/> : p.price}</td><td>{editingId===p.id ? <select value={editForm.categoryIds[0] || ''} onChange={(e)=>setEditForm({...editForm,categoryIds:e.target.value ? [Number(e.target.value)] : []})}><option value="">Категория</option>{categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select> : (p.categories||[]).map(c=><span className="badge category" key={c.id}>{c.name}</span>)}</td><td className="actions">{editingId===p.id ? <button className="btn btn-edit" onClick={()=>saveEdit(p.id)}>Save</button> : <button className="btn btn-edit" onClick={()=>startEdit(p)}>Edit</button>} <button className="btn btn-delete" onClick={()=>remove(p.id)}>Delete</button></td></tr>)}
        </tbody></table>
    </div>;
}